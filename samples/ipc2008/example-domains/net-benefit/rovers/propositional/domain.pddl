(define (domain Rover)
(:requirements :typing :durative-actions :fluents :duration-inequalities :action-costs :goal-utilities)
...
(:functions
	...
	(traverse_cost ?r - rover ?f - waypoint ?t - waypoint) - number
	(total-cost) - number)
...
:effect (and (at start (decrease (energy ?x) 8))
	(at start (not (at ?x ?y)))
	(at end (at ?x ?z))
	(at end (increase (total-cost) (traverse_cost ?x ?y ?z))))
)
